 ...
 */
export class DataSource extends DisposableBase {
	 ...
	 */
	constructor(gitExecutable: GitExecutable | null, onDidChangeConfiguration: Event<unknown>, onDidChangeGitExecutable: Event<GitExecutable>, logger: Logger) {
		super();
		this.logger = logger;
		...
		this.registerDisposables(
			onDidChangeConfiguration(() => {
				// Just regenerate formats on any configuration change
	...
	public getCommitComparison(repo: string, fromHash: string, toHash: string): Promise<GitCommitComparisonData> {
		return Promise.all([
			...
			return {
				fileChanges: generateFileChanges(results[0] as DiffNameStatusRecord[], results[1] as DiffNumStatRecord[], results[2] as GitStatusFiles | null),
				error: null
			};
		}).catch((errorMessage) => {
				...
				if (!err) {
					...
					for (let i = 0; i < lines.length; i++) {
						...
						if (inSubmoduleSection && (match = lines[i].match(pathProp)) !== null) {
							let root = await this.repoRoot(getPathFromUri(path.join(repo, getPathFromStr(match[1]))));
	...
	public repoRoot(pathOfPotentialRepo: string) {
		return this.spawnGit(['rev-parse', '--show-toplevel'], pathOfPotentialRepo, (stdout) => getPathFromUri(path.normalize(stdout.trim()))).then(async (pathReturnedByGit) => {
	...
	private _spawnGit<T>(args: string[], repo: string, resolveValue: { (stdout: Buffer, stderr: string): T }, ignoreExitCode: boolean = false) {
		return new Promise<T>((resolve, reject) => {
			...
			let timeoutId = setTimeout(() => {
				child.kill();
				reject('Git command timed out after 30 seconds.');
			}, 30000);

			resolveSpawnOutput(child).then((values: any) => {
